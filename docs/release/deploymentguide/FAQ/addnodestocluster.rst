...
For example, if there are 3 nodes with the label ``Management`` - update the custom-management.yaml::
    ...
    curl -s -o custom-management.yaml https://raw.githubusercontent.com/EricomSoftwareLtd/Shield/Rel-20.02/Kube/scripts/custom-management.yaml

Or, if there are 3 nodes with the label ``farm-services`` - update the custom-farm.yaml::

    curl -s -o custom-farm.yaml https://raw.githubusercontent.com/EricomSoftwareLtd/Shield/Rel-20.02/Kube/scripts/custom-farm.yaml
